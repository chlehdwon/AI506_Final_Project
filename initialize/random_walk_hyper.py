...
def random_walk_hyper(args, node_list, hyperedge_list):
    ...
    walks_save_path = '../walks/{}/p{}_q{}_r{}_l{}_hyper_walks.txt'.format(
        args.dataset_name, p, q, num_walks, walk_length)
    
    if not os.path.exists("../walks"):
        os.mkdir("../walks")
        
    if not os.path.exists("../walks/{}/".format(args.dataset_name)):
        os.mkdir("../walks/{}/".format(args.dataset_name))
    start = time.time()

    if False:
    #not args.TRY and os.path.exists(walks_save_path):
